...
$(window).scroll(function(){
    ...
    if(wScroll > $('.flyincontainer1').offset().top - $(window).height()){
        var offset = (Math.min(0, wScroll - $('.flyin1').offset().top + $(window).height() - 380)).toFixed();
        $('.flyin1').css({'transform': 'translate(' + offset + 'px, ' + Math.abs(offset * 0.2) + 'px)'});
    }

    if(wScroll > $('.flyincontainer2').offset().top - $(window).height()){
        var offset = (Math.min(0, wScroll - $('.flyin2').offset().top + $(window).height() - 380)).toFixed();
        $('.flyin2').css({'transform': 'translate(' + Math.abs(offset) + 'px, ' + Math.abs(offset * 0.2) + 'px)'});
    }
